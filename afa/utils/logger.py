...
from .structures import ArgsType

logger = logging.getLogger("pytorch_lightning")
warnings.filterwarnings("ignore", category=DeprecationWarning)


class AFAProgressBar(pl.callbacks.ProgressBarBase):  # type: ignore
...
def setup_logger(
    ...
    """Configure logging for AFA using the pytorch lightning logger."""
    ...
    plain_formatter = logging.Formatter(
        "[%(asctime)s] AFA %(levelname)s: %(message)s",
        datefmt="%m/%d %H:%M:%S",
    )
    ...
    if color:
        formatter = _ColorFormatter(
            colored("[%(asctime)s AFA]: ", "green") + "%(message)s",
            datefmt="%m/%d %H:%M:%S",
        )
